...
class HetAD(nn.Module):
    ...
    def __init__(self, embed_dim, uu_neigh, ui_neigh, iu_neigh, ii_neigh):
        ...
        self.uu_neigh_rnn = nn.LSTM(embed_dim, int(embed_dim / 2), 1, bidirectional=True)
        self.ui_neigh_rnn = nn.LSTM(embed_dim, int(embed_dim / 2), 1, bidirectional=True)
        self.iu_neigh_rnn = nn.LSTM(embed_dim, int(embed_dim / 2), 1, bidirectional=True)
        self.ii_neigh_rnn = nn.LSTM(embed_dim, int(embed_dim / 2), 1, bidirectional=True)
        #
        ...
        self.i_i_embed = np.zeros((args.i_l + 1, embed_dim))
        ...
        for item in ii_neigh:
            normal_item = item[1:]
            items = ii_neigh[item][1: -1].split(', ')
            for neigh_item in items:
                neigh_item = neigh_item[1: -1]
                self.i_i_embed[int(normal_item)] = np.add(self.i_i_embed[int(normal_item)], self.i_self_embed[int(neigh_item[1:])])
            if len(items) != 0:
                self.i_i_embed[int(normal_item)] = self.i_i_embed[int(normal_item)] / len(items)
            else:
                self.i_i_embed[int(normal_item)] = self.i_self_embed[int(normal_item)]
    ...
    def u_content_agg(self, id_batch):
        ...
        concat_embed = torch.tensor(u_self_embed).view(len(u_self_embed), 1, self.embed_dim)
        # att_w = self.att(concat_embed)

        # concat_embed = u_self_embed
    ...
    def i_content_agg(self, id_batch):
        ...
        concat_embed = torch.tensor(i_self_embed).view(len(i_self_embed), 1, self.embed_dim)
        # concat_embed = i_self_embed
        concat_embed = torch.transpose(concat_embed, 0, 1)
        concat_embed = torch.tensor(concat_embed, dtype=torch.float32)
        all_state, last_state = self.u_content_rnn(concat_embed)
    ...
    def node_neigh_agg(self, id_batch, type):
        if type == 1:
            ui_neigh_embed = self.u_i_embed[id_batch]
            uu_neigh_embed = self.u_u_embed[id_batch]
            ui_neigh_embed = torch.from_numpy(ui_neigh_embed).view(len(ui_neigh_embed), 1, self.embed_dim)
            uu_neigh_embed = torch.from_numpy(uu_neigh_embed).view(len(uu_neigh_embed), 1, self.embed_dim)
            uu_neigh_embed = torch.transpose(uu_neigh_embed, 0, 1)
            uu_neigh_embed = torch.tensor(uu_neigh_embed, dtype=torch.float32)
            ui_neigh_embed = torch.transpose(ui_neigh_embed, 0, 1)
            ui_neigh_embed = torch.tensor(ui_neigh_embed, dtype=torch.float32)

            all_state, last_state = self.uu_neigh_rnn(uu_neigh_embed)
            all_state1, last_state1 = self.ui_neigh_rnn(ui_neigh_embed)
            return torch.mean(all_state, 0), torch.mean(all_state1, 0)


        elif type == 2:
            iu_neigh_embed = self.i_u_embed[id_batch]
            ii_neigh_embed = self.i_i_embed[id_batch]
            iu_neigh_embed = torch.from_numpy(iu_neigh_embed).view(len(iu_neigh_embed), 1, self.embed_dim)
            ii_neigh_embed = torch.from_numpy(ii_neigh_embed).view(len(ii_neigh_embed), 1, self.embed_dim)
            iu_neigh_embed = torch.transpose(iu_neigh_embed, 0, 1)
            iu_neigh_embed = torch.tensor(iu_neigh_embed, dtype=torch.float32)
            ii_neigh_embed = torch.transpose(ii_neigh_embed, 0, 1)
            ii_neigh_embed = torch.tensor(ii_neigh_embed, dtype=torch.float32)

            all_state, last_state = self.iu_neigh_rnn(iu_neigh_embed)
            all_state1, last_state1 = self.ii_neigh_rnn(ii_neigh_embed)
            return torch.mean(all_state, 0), torch.mean(all_state1, 0)
    ...
    def types_agg(self, id_batch, type):
        ...
        if type == 1:
            ...
            # uu          ui
            c_agg_neigh, c1_agg_neigh = self.node_neigh_agg(id_batch, type)
            u_self_agg = torch.cat((c_agg_deputy, c_agg_deputy), 1).view(len(c_agg_deputy), self.embed_dim * 2)
            uu_agg = torch.cat((c_agg_deputy, c_agg_neigh), 1).view(len(c_agg_deputy), self.embed_dim * 2)
            ui_agg = torch.cat((c_agg_deputy, c1_agg_neigh), 1).view(len(c_agg_deputy), self.embed_dim * 2)

            concat_embed = torch.cat((u_self_agg, uu_agg, ui_agg), 1).view(len(c_agg_deputy), 3, self.embed_dim * 2)
            atten_w = self.act(torch.bmm(concat_embed, self.u_neigh_att.unsqueeze(0).expand(len(c_agg_deputy),*self.u_neigh_att.size())))
            atten_w = self.softmax(atten_w).view(len(c_agg_deputy), 1, 3)
            # 这里要跟上面对应好
            concat_embed = torch.cat((c_agg_deputy, c_agg_neigh, c1_agg_neigh), 1).view(len(c_agg_deputy), 3, self.embed_dim)
            agg_batch = torch.bmm(atten_w, concat_embed).view(len(c_agg_deputy), self.embed_dim)
            ...
            # iu              ii
            c_agg_neigh, c1_agg_neigh = self.node_neigh_agg(id_batch, type)
            i_self_agg = torch.cat((c_agg_deputy, c_agg_deputy), 1).view(len(c_agg_deputy), self.embed_dim * 2)
            iu_agg = torch.cat((c_agg_deputy, c_agg_neigh), 1).view(len(c_agg_deputy), self.embed_dim * 2)
            ii_agg = torch.cat((c_agg_deputy, c1_agg_neigh), 1).view(len(c_agg_deputy), self.embed_dim * 2)

            concat_embed = torch.cat((i_self_agg, iu_agg, ii_agg), 1).view(len(c_agg_deputy), 3, self.embed_dim * 2)
            atten_w = self.act(torch.bmm(concat_embed, self.i_neigh_att.unsqueeze(0).expand(len(c_agg_deputy),*self.i_neigh_att.size())))
            atten_w = self.softmax(atten_w).view(len(c_agg_deputy), 1, 3)
...
def cross_entropy_loss(uu_embed_batch, ui_embed_batch, iu_embed_batch, ii_embed_batch, embed_d):
    ...
    c_ii_embed_batch = ii_embed_batch[0]
    p_ii_embed_batch = ii_embed_batch[1]
    n_ii_embed_batch = ii_embed_batch[2]


    batch_size = c_uu_embed_batch.shape[0] + c_ui_embed_batch.shape[0] + c_iu_embed_batch.shape[0] + c_ii_embed_batch.shape[0]

    c_embed_batch = torch.cat([c_uu_embed_batch, c_ui_embed_batch, c_iu_embed_batch, c_ii_embed_batch], axis = 0).view(batch_size, 1, embed_d)
    p_embed_batch = torch.cat([p_uu_embed_batch, p_ui_embed_batch, p_iu_embed_batch, p_ii_embed_batch], axis = 0).view(batch_size, embed_d, 1)
    n_embed_batch = torch.cat([n_uu_embed_batch, n_ui_embed_batch, n_iu_embed_batch, n_ii_embed_batch], axis = 0).view(batch_size, embed_d, 1)
